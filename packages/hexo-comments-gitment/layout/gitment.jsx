const { Component } = require('inferno');

module.exports = class extends Component {
    render() {
        const { theme, helper } = this.props;
        // 获取资源标签的 HTML 字符串
        const gitmintEnabled = theme.gitment && theme.gitment.gitmint;
        const gitmintCSS = gitmintEnabled
            ? '<link rel="stylesheet" href="' + (theme.gitment.gitmint_css || 'https://cdn.jsdelivr.net/npm/gitmint@0.0.3-update.3/style/default.css') + '">'
            : '<link rel="stylesheet" href="https://imsun.github.io/gitment/style/default.css">';
        const gitmentCSSData = gitmintCSS;

        const gitmentCSSData1 = 
            `<style>
                :root {
                    --gm-bg-color: #fff;
                    --gm-bg-secondary-color: #f5f5f5;
                    --gm-bg-tertiary-color: #fafbfc;
                    --gm-text-secondary-color: #666;
                    --gm-comment-header-bg: #fff;
                    --gm-comment-header-color: #666;
                    --gm-text-heading-color: #1b1f23;
                    --gm-border-color: #cfd8dc;
                    --gm-btn-primary-bg: #00BCD4;
                    --gm-btn-primary-hover-bg: #00ACC1;
                    --gm-btn-primary-disabled-bg: #4DD0E1;
                    --gm-btn-primary-text: #fff;
                    --gm-like-color: #F44336;
                    --gm-footer-tip-hover-color: #2196F3;
                    --gm-spinner-fill: #333;
                    --gm-code-bg: rgba(27,31,35,0.05);
                    --gm-code-block-bg: #f5f5f5;
                    --gm-table-border: #dfe2e5;
                    --gm-kbd-border: #c6cbd1;
                    --gm-kbd-border-bottom: #959da5;
                    --gm-kbd-text: #444d56;
                    --gm-kbd-shadow: inset 0 -1px 0 #959da5;
                    --gm-blockquote-color: #6a737d;
                    --gm-blockquote-border: #dfe2e5;
                    --gm-hr-color: #e1e4e8;
                    --gm-h1-h2-border: #eaecef;
                    --gm-github-icon-fill: #fff;
                    --gm-github-icon-bg: #333;
                    --gm-comment-arrow-bg: #fff;
                    --gm-fold-gradient-end: rgba(255, 255, 255, .9);
                    --gm-header-textarea-bg-color: #fff;
                    --gm-header-textarea-border-color: #d1d5da;
                    --gm-comment-bg-color: #f9f9f9;
                    --gm-comment-body-color: #333;
                    --gm-admin-comment-bg-color: #f6f9fe;
                    --gm-btn-bg-color: #fff;
                    --gm-btn-hover-bg-color: #f3f4f6;
                    --gm-btn-border-color: #d1d5da;
                    color-scheme: light;
                }

                .dark-theme {
                    --gm-bg-color: #1a1a1a;
                    --gm-bg-secondary-color: #2a2a2a;
                    --gm-bg-tertiary-color: #333;
                    --gm-text-secondary-color: #999;
                    --gm-comment-header-bg: #444;
                    --gm-comment-header-color: #ccc;
                    --gm-text-heading-color: #e1e4e8;
                    --gm-border-color: #666;
                    --gm-btn-primary-bg: #0088a3;
                    --gm-btn-primary-hover-bg: #007a91;
                    --gm-btn-primary-disabled-bg: #3a6b7a;
                    --gm-btn-primary-text: #fff;
                    --gm-like-color: #F44336;
                    --gm-footer-tip-hover-color: #2196F3;
                    --gm-spinner-fill: #e1e4e8;
                    --gm-code-bg: rgba(240,246,252,0.15);
                    --gm-code-block-bg: #161b22;
                    --gm-table-border: #30363d;
                    --gm-kbd-border: #555;
                    --gm-kbd-border-bottom: #555;
                    --gm-kbd-text: #e1e4e8;
                    --gm-kbd-shadow: inset 0 -1px 0 #555;
                    --gm-blockquote-color: #8b949e;
                    --gm-blockquote-border: #3b434b;
                    --gm-hr-color: #30363d;
                    --gm-h1-h2-border: #666;
                    --gm-github-icon-fill: #e1e4e8;
                    --gm-github-icon-bg: #333;
                    --gm-comment-arrow-bg: #1a1a1a;
                    --gm-fold-gradient-end: rgba(26, 26, 26, .9);
                    --gm-header-textarea-bg-color: #333;
                    --gm-header-textarea-border-color: #555;
                    --gm-comment-bg-color: #252525;
                    --gm-comment-body-color: #e1e4e8;
                    --gm-admin-comment-bg-color: #1a2535;
                    --gm-btn-bg-color: #444;
                    --gm-btn-hover-bg-color: #333;
                    --gm-btn-border-color: #555;
                    color-scheme: dark;
                }

                .gitment-container {
                    color: var(--text-color) !important;
                }

                    /* Links */
                .gitment-container a,
                .gitment-container a:visited {
                    color: var(--text-color) !important;
                    border-bottom: 0;
                }

                .gitment-container a:hover {
                    color: var(--text-color) !important;
                }

                    /* Spinner */
                .gitment-container .gitment-spinner-icon {
                    fill: var(--gm-spinner-fill) !important;
                }

                    /* Header */
                .gitment-container .gitment-header {
                    border-color: var(--gm-border-color) !important;
                }

                .gitment-container .gitment-header-issue-link,
                .gitment-container .gitment-header-issue-link:visited,
                .gitment-container .gitment-header-issue-link:hover {
                    color: var(--gm-text-secondary-color) !important;
                }

                    /* Buttons */
                .gitment-container .gitment-btn {
                    background-color: var(--gm-btn-bg-color) !important;
                    border-color: var(--gm-btn-border-color) !important;
                    color: var(--text-color) !important;
                }

                .gitment-container .gitment-btn:hover {
                    background-color: var(--gm-btn-hover-bg-color) !important;
                }

                .gitment-container .gitment-editor-submit,
                .gitment-container .gitment-comments-init-btn {
                    color: var(--gm-btn-primary-text) !important;
                    background-color: var(--gm-btn-primary-bg) !important;
                }

                .gitment-container .gitment-editor-submit:hover,
                .gitment-container .gitment-comments-init-btn:hover {
                    background-color: var(--gm-btn-primary-hover-bg) !important;
                }

                .gitment-container .gitment-editor-submit:disabled,
                .gitment-container .gitment-comments-init-btn:disabled {
                    color: rgba(255,255,255,0.75) !important;
                    background-color: var(--gm-btn-primary-disabled-bg) !important;
                }

                    /* Like button */
                .gitment-container .gitment-comment-like-btn,
                .gitment-container .gitment-comment-like-btn svg,
                .gitment-container .gitment-header-like-btn svg {
                    color: var(--text-color) !important;
                    fill: var(--text-color) !important;
                }

                .gitment-container .gitment-comment-like-btn.liked,
                .gitment-container .gitment-comment-like-btn.liked svg,
                .gitment-container .gitment-header-like-btn.liked svg {
                    color: var(--gm-like-color) !important;
                    fill: var(--gm-like-color) !important;
                }

                    /* Comment */
                .gitment-container .gitment-comment {
                    border-color: var(--gm-border-color) !important;
                }

                .gitment-container .gitment-comment-main,
                .gitment-container .gitment-comment-body {
                    color: var(--gm-comment-body-color) !important;
                }

                .gitment-container .gitment-comment-body {
                    background-color: var(--gm-comment-bg-color) !important;
                }

                .gitment-container .gitment-admin-comment .gitment-comment-body {
                    background-color: var(--gm-admin-comment-bg-color) !important;
                }

                .gitment-container .gitment-comment-main {
                    border-color: var(--gm-border-color) !important;
                }

                .gitment-container .gitment-comment-main::before {
                    border-right-color: var(--gm-border-color) !important;
                }

                .gitment-container .gitment-comment-main::after {
                    border-right-color: var(--gm-comment-arrow-bg) !important;
                }

                .gitment-container .gitment-comment-header {
                    color: var(--gm-comment-header-color) !important;
                    background-color: var(--gm-comment-header-bg) !important;
                }

                .gitment-container a.gitment-comment-name,
                .gitment-container a.gitment-comment-name:visited {
                    color: var(--gm-text-secondary-color) !important;
                }

                    /* Comment body folded */
                .gitment-container .gitment-comment-body-folded::before {
                    background: -webkit-linear-gradient(top, transparent, var(--gm-fold-gradient-end)) !important;
                    background: linear-gradient(180deg, transparent, var(--gm-fold-gradient-end)) !important;
                }

                .gitment-container .gitment-comment-body-folded::after {
                    color: var(--gm-text-secondary-color) !important;
                    background: var(--gm-fold-gradient-end) !important;
                }

                    /* Editor */
                .gitment-container .gitment-editor-container {
                    color: var(--text-color) !important;
                }

                .gitment-container .gitment-editor-main {
                    border-color: var(--gm-border-color) !important;
                }

                .gitment-container .gitment-editor-main::before {
                    border-right-color: var(--gm-border-color) !important;
                }

                .gitment-container .gitment-editor-main::after {
                    border-right-color: var(--gm-comment-arrow-bg) !important;
                }

                .gitment-container .gitment-editor-header {
                    border-bottom-color: var(--gm-border-color) !important;
                }

                .gitment-container .gitment-editor-tab {
                    color: var(--gm-text-secondary-color) !important;
                }

                .gitment-container .gitment-editor-tab.gitment-selected {
                    color: var(--text-color) !important;
                    background-color: var(--gm-bg-color) !important;
                    border-color: var(--gm-border-color) !important;
                }

                .gitment-container .gitment-editor-body textarea {
                    background-color: var(--gm-header-textarea-bg-color) !important;
                    color: var(--text-color) !important;
                    border-color: var(--gm-header-textarea-border-color) !important;
                }

                .gitment-container .gitment-editor-body textarea:focus {
                    background-color: var(--gm-header-textarea-bg-color) !important;
                }

                .gitment-container .gitment-editor-preview {
                    color: var(--text-color) !important;
                    background-color: var(--gm-header-textarea-bg-color) !important;
                }

                .gitment-container .gitment-editor-footer-tip {
                    color: var(--gm-text-secondary-color) !important;
                }

                .gitment-container a.gitment-editor-footer-tip:hover {
                    color: var(--gm-footer-tip-hover-color) !important;
                }

                .gitment-container a.gitment-editor-login-link,
                .gitment-container a.gitment-editor-login-link:visited {
                    color: var(--text-color) !important;
                }

                .gitment-container a.gitment-editor-login-link:hover {
                    color: var(--gm-btn-primary-hover-bg) !important;
                }

                .gitment-container a.gitment-editor-logout-link,
                .gitment-container a.gitment-editor-logout-link:visited {
                    color: var(--gm-text-secondary-color) !important;
                }

                .gitment-container a.gitment-editor-logout-link:hover {
                    color: var(--gm-btn-primary-disabled-bg) !important;
                }

                    /* Footer */
                .gitment-container .gitment-footer {
                    border-color: var(--gm-border-color) !important;
                }

                .gitment-container a.gitment-footer-project-link,
                .gitment-container a.gitment-footer-project-link:visited {
                    color: var(--text-color) !important;
                }

                    /* Comments list */
                .gitment-container .gitment-comments-empty,
                .gitment-container .gitment-comments-loading,
                .gitment-container .gitment-comments-error,
                .gitment-container .gitment-comments-null {
                    color: var(--text-color) !important;
                }

                    /* Pagination */
                .gitment-container .gitment-comments-page-item {
                    color: var(--text-color) !important;
                    border-color: var(--gm-border-color) !important;
                }

                .gitment-container .gitment-comments-page-item:hover,
                .gitment-container .gitment-comments-page-item.gitment-selected {
                    background-color: var(--gm-bg-secondary-color) !important;
                }

                    /* Markdown content */
                .gitment-container .gitment-markdown {
                    color: var(--text-color) !important;
                }

                .gitment-container .gitment-markdown a {
                    color: var(--text-color) !important;
                }

                .gitment-container .gitment-markdown hr {
                    background-color: var(--gm-hr-color) !important;
                    border-bottom-color: var(--gm-hr-color) !important;
                }

                .gitment-container .gitment-markdown h1,
                .gitment-container .gitment-markdown h2 {
                    border-bottom-color: var(--gm-h1-h2-border) !important;
                }

                .gitment-container .gitment-markdown h1,
                .gitment-container .gitment-markdown h2,
                .gitment-container .gitment-markdown h3,
                .gitment-container .gitment-markdown h4,
                .gitment-container .gitment-markdown h5,
                .gitment-container .gitment-markdown h6 {
                    color: var(--text-color) !important;
                }

                .gitment-container .gitment-markdown h6 {
                    color: var(--gm-text-secondary-color) !important;
                }

                .gitment-container .gitment-markdown blockquote {
                    color: var(--gm-blockquote-color) !important;
                    border-left-color: var(--gm-blockquote-border) !important;
                }

                .gitment-container .gitment-markdown code {
                    background-color: var(--gm-code-bg) !important;
                    color: var(--text-color) !important;
                }

                .gitment-container .gitment-markdown pre,
                .gitment-container .gitment-markdown .highlight pre {
                    background-color: var(--gm-code-block-bg) !important;
                }

                .gitment-container .gitment-markdown pre > code {
                    background-color: transparent !important;
                }

                .gitment-container .gitment-markdown table tr {
                    background-color: var(--gm-bg-color) !important;
                    border-top-color: var(--gm-table-border) !important;
                }

                .gitment-container .gitment-markdown table tr:nth-child(2n) {
                    background-color: var(--gm-bg-secondary-color) !important;
                }

                .gitment-container .gitment-markdown table th,
                .gitment-container .gitment-markdown table td {
                    border-color: var(--gm-table-border) !important;
                }

                .gitment-container .gitment-markdown kbd {
                    color: var(--gm-kbd-text) !important;
                    background-color: var(--gm-bg-tertiary-color) !important;
                    border-color: var(--gm-kbd-border) !important;
                    border-bottom-color: var(--gm-kbd-border-bottom) !important;
                    box-shadow: var(--gm-kbd-shadow) !important;
                }

                .gitment-container .gitment-markdown .pl-s,
                .gitment-container .gitment-markdown .pl-pds,
                .gitment-container .gitment-markdown .pl-s .pl-pse .pl-s1,
                .gitment-container .gitment-markdown .pl-sr,
                .gitment-container .gitment-markdown .pl-sr .pl-cce,
                .gitment-container .gitment-markdown .pl-sr .pl-sre,
                .gitment-container .gitment-markdown .pl-sr .pl-sra {
                    color: var(--text-color) !important;
                }

                .gitment-container .gitment-markdown .pl-smi,
                .gitment-container .gitment-markdown .pl-s .pl-s1,
                .gitment-container .gitment-markdown .pl-mi,
                .gitment-container .gitment-markdown .pl-mb {
                    color: var(--text-color) !important;
                }

                .gitment-container .gitment-markdown .pl-ii,
                .gitment-container .gitment-markdown .pl-c2 {
                    color: #f8f8f8 !important;
                }

                .gitment-container .gitment-markdown img {
                    background-color: var(--gm-bg-color) !important;
                }

                    /* GitHub icon avatar */
                .gitment-container .gitment-editor-avatar .gitment-github-icon {
                    fill: var(--gm-github-icon-fill) !important;
                    background-color: var(--gm-github-icon-bg) !important;
                }

            </style>`;

        const gitmentConfigData = helper.diversity_data('gitment', theme.gitment, {
            js : 'https://imsun.github.io/gitment/dist/gitment.browser.js'
        }).toString();

        const gitmentJSData = 
            `<script>
            (function() {
                const loadGitment = () => {
                    // 尝试从 localStorage 读取已存储的 token
                    let preinitToken = null;
                    try {
                        preinitToken = Diversity.data.get('gitment-comments-token');
                    } catch (e) {}

                    // ===== XHR 劫持 =====
                    // 尝试从隐藏 iframe 获取真正原生的 XMLHttpRequest.open。
                    if (!preinitToken && conf.gitment.proxy) {
                        var NativeXHR, nativeOpen;
                        try {
                            var frame = document.createElement('iframe');
                            frame.style.display = 'none';
                            frame.src = 'about:blank';
                            document.body.appendChild(frame);
                            NativeXHR = frame.contentWindow.XMLHttpRequest;
                            nativeOpen = NativeXHR.prototype.open;
                            // 不能 removeChild！NativeXHR 实例绑定在 iframe 的 document 上。
                        } catch(e) {
                            NativeXHR = window.XMLHttpRequest;
                            nativeOpen = NativeXHR.prototype.open;
                        }
                        if (!nativeOpen) { return; }

                        window.XMLHttpRequest = function PatchedXHR() {
                            var xhr = new NativeXHR();
                            xhr.open = function(method, url) {
                                if (url && url.indexOf('gh-oauth.imsun.net') !== -1) {
                                    return nativeOpen.call(this, method, conf.gitment.proxy);
                                }
                                return nativeOpen.call(this, method, url);
                            };
                            return xhr;
                        };
                        ['UNSENT', 'OPENED', 'HEADERS_RECEIVED', 'LOADING', 'DONE'].forEach(function(k) {
                            window.XMLHttpRequest[k] = NativeXHR[k];
                        });
                    }

                    const loadingElement = document.getElementById('loading-gitment');
                    // 配色方案刷新，重新加载评论
                    // 加载提示被隐藏，需要再次显示
                    loadingElement?.classList.remove('hidden');

                    const useGitmint = conf.gitment.gitmint;
                    const gitmintJsDefault = 'https://cdn.jsdelivr.net/npm/gitmint@0.0.3-update.3/dist/gitmint.browser.js';
                    const scriptUrl = useGitmint ? (conf.gitment.gitmint_js || gitmintJsDefault) : conf.gitment.js;
                    const commentClass = useGitmint ? 'Gitmint' : 'Gitment';

                    const wrapSelector = '.gitment-wrap';
                    Diversity.utils.loadComments(wrapSelector)
                        .then(() => Diversity.utils.getScript(scriptUrl, {
                            condition: window[commentClass]
                        }))
                        .then(() => {
                            let issueTerm = conf.gitment.issue_term;
                            let gitmentId;
                            if (issueTerm === 'pathname')
                                gitmentId = window.location.pathname;
                            else if (issueTerm === 'url')
                                gitmentId = window.location.href;
                            else if (issueTerm === 'title')
                                gitmentId = document.title;
                            else
                                gitmentId = issueTerm || conf.gitment.id || window.location.href;

                            let confgObj = {
                                owner: conf.gitment.owner,
                                repo: conf.gitment.repo,
                                oauth: {
                                    client_id: conf.gitment.client_id,
                                    client_secret: conf.gitment.client_secret
                                },
                                id: gitmentId,
                                perPage: conf.gitment.per_page || 20,
                                maxCommentHeight: conf.gitment.max_comment_height || 250
                            }
                            if (conf.gitment.title)
                                confgObj.title = conf.gitment.title;
                            if (conf.gitment.link)
                                confgObj.link = conf.gitment.link;
                            if (conf.gitment.desc)
                                confgObj.desc = conf.gitment.desc;
                            if (conf.gitment.labels && Array.isArray(conf.gitment.labels) && conf.gitment.labels.length > 0)
                                confgObj.labels = conf.gitment.labels;
                            if (conf.gitment.lang)
                                confgObj.lang = conf.gitment.lang;

                            // 如果有 token，直接注入，表示通过外部进行 OAuth 流程【Diversity Comments 使用】
                            if (preinitToken) {
                                confgObj.oauth.token = preinitToken;
                            }

                            const CommentClass = window[commentClass];
                            const instance = new CommentClass(confgObj);
                            instance.render(document.querySelector(wrapSelector));

                            if (loadingElement && document.querySelector('.gitment-container')) {
                                loadingElement.classList.add('hidden');
                            }
                        });

                    // Gitment加载检测（Gitment通过DOM渲染，非iframe方案）
                    const observeGitmentLoaded = () => {
                        const container = document.querySelector('.gitment-wrap');
                        if (!container) return;
                        const observer = new MutationObserver((mutations, obs) => {
                            if (container.querySelector('.gitment-container')) {
                                loadingElement?.classList.add('hidden');
                                obs.disconnect();
                            }
                        });
                        observer.observe(container, { childList: true, subtree: true });
                    };
                    observeGitmentLoaded();
                }

                document.addEventListener('page:loaded', loadGitment);
                document.addEventListener('color-scheme:refresh', loadGitment);
            })();
            </script>`;

        return (
            <div dangerouslySetInnerHTML={{ __html: gitmentCSSData + gitmentCSSData1 + gitmentConfigData + gitmentJSData}}></div>
        );
    }
}

