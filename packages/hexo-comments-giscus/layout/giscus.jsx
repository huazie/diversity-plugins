const { Component } = require('inferno');

module.exports = class extends Component {
    render() {
        const { theme, helper } = this.props;

        const giscusConfigData = helper.diversity_data('giscus', theme.giscus, {
            js : 'https://giscus.app/client.js'
        }).toString();

        const giscusJSData = 
            `<script>
            (function() {
                // Giscus 加载完成 → 隐藏 loading
                // Giscus OAuth 完成后会重新加载 iframe，触发 message 消息
                window.addEventListener('message', (e) => {
                    if (!e.data || typeof e.data !== 'object') return;
                    // Giscus 加载完成（数据中存在 giscus 字段）→ 隐藏 loading
                    if (e.data.giscus) {
                        document.getElementById('loading-giscus')?.classList.add('hidden');
                    }
                });

                const loadGiscus = () => {
                    const loadingElement = document.getElementById('loading-giscus');
                    // 配色方案刷新，重新加载评论
                    // 加载提示被隐藏，需要再次显示
                    loadingElement?.classList.remove('hidden');

                    // 清除旧的 Giscus iframe（如果存在）
                    const wrap = document.querySelector('.giscus-wrap');
                    if (wrap) {
                        const oldIframe = wrap.querySelector('iframe.giscus-frame');
                        if (oldIframe) {
                            oldIframe.remove();
                        }
                        const oldScript = wrap.querySelector('script[src*="giscus.app"]');
                        if (oldScript) {
                            oldScript.remove();
                        }
                    }

                    let lang;
                    // 加载评论模块
                    Diversity.utils.loadComments('.giscus-wrap')
                        .then(() => {
                            lang = conf.giscus.lang;
                            if (!lang)
                                lang = window.navigator.language;
                        })
                        .then(() => {
                            const attrs = {
                                async                   : true,
                                crossOrigin             : 'anonymous',
                                'data-repo'             : conf.giscus.repo,
                                'data-repo-id'          : conf.giscus.repo_id,
                                'data-category'         : conf.giscus.category,
                                'data-category-id'      : conf.giscus.category_id,
                                'data-mapping'          : conf.giscus.mapping,
                                'data-term'             : conf.giscus.term,
                                'data-strict'           : conf.giscus.strict,
                                'data-reactions-enabled': conf.giscus.reactions_enabled,
                                'data-emit-metadata'    : conf.giscus.emit_metadata,
                                'data-theme'            : Diversity.utils.isDarkMode() ? conf.giscus.dark : conf.giscus.theme,
                                'data-lang'             : lang,
                                'data-input-position'   : conf.giscus.input_position,
                                'data-loading'          : conf.giscus.data_loading
                            };
                            return Diversity.utils.getScript(conf.giscus.js, {
                                attributes: attrs,
                                parentNode: document.querySelector('.giscus-wrap')
                            });
                        });
                }

                document.addEventListener('page:loaded', loadGiscus);
                document.addEventListener('color-scheme:refresh', loadGiscus);
            })();
            </script>`;

        return (
            <div dangerouslySetInnerHTML={{ __html: giscusConfigData + giscusJSData}}></div>
        );
    }
}